  ...
  if (client) { // Если подключается новый клиент,
    ...
    while (client.connected()) { // цикл, пока есть соединение клиента
      if (client.available()) { // если от клиента поступают данные,
       ...
       //       EEPROM.write(BTN_1, output26State);
       //       EEPROM.commit();
       ...
       //       EEPROM.write(BTN_2, output27State);
       //       EEPROM.commit();
         ...
         //     EEPROM.commit();
            ...
            client.println("<input type=\"range\" min=\"-10\" max=\"10\">");
            EEPROM.get(BTN_1, output26State);
            ...
            EEPROM.get(BTN_2, output27State);
